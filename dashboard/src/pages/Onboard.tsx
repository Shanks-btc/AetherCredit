...
export function Onboard() {
  ...
  const agentTypes: { type: AgentType; icon: string; desc: string }[] = [
    { type: 'Trading',           icon: '📈', desc: 'Autonomous on-chain trading and execution' },
  ...
  const plans = [
    ...
    },
    {
      ...
      popular: true,
      features: ['Full credit analytics', 'Unlimited API calls', 'Up to 0.5 OG credit', 'Priority support'],
    },
    {
      id:      'protocol' as Plan,
      name:    'Protocol Fleet',
      ...
      popular: false,
      features: ['Multi-agent management', 'Up to 2 OG credit', 'SLA guarantee'],
    },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white flex flex-col">
      ...
      {/* Navbar */}
      <nav className="border-b border-slate-800 px-4 sm:px-6 py-3 flex items-center justify-between bg-[#0a0f1e]/90 backdrop-blur-sm">
        ...
        <button onClick={() => navigate('/')} className="text-xs text-slate-500 hover:text-white transition-colors">
          ← Back
        </button>
      </nav>
      ...
      <div className="flex-1 flex flex-col items-center px-4 py-6 sm:py-10">
        ...
        <div className="mb-6 text-center">
          <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-2xl mx-auto mb-3">
            {step === 4 ? '✅' : '🤖'}
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-white mb-1">Create Agent</h1>
          <p className="text-slate-500 text-xs sm:text-sm">Register your autonomous agent on the AetherCredit 0G network</p>
        </div>
          ...
          {steps.map((s, i) => (
            <div key={s.id} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`h-7 w-7 sm:h-9 sm:w-9 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
                  step === s.id
                ...
                <span className={`text-[9px] sm:text-[10px] mt-1 font-semibold tracking-widest hidden sm:block ${
              ...
              {i < steps.length - 1 && (
                <div className={`h-px w-8 sm:w-16 mb-0 sm:mb-4 mx-1 transition-all ${step > s.id ? 'bg-indigo-600' : 'bg-slate-700'}`} />
              )}
            </div>
          ))}
        </div>
          ...
          {step === 1 && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="p-5 sm:p-8">
                ...
                <h2 className="text-lg sm:text-xl font-bold text-white mb-2">Connect Your Agentic Wallet</h2>
                <p className="text-slate-400 text-xs sm:text-sm mb-6 leading-relaxed">
                  Provide your email to create or link an Agentic Wallet. This identity is used to manage your agent on-chain activity, subscriptions, and credit score.
                </p>
                <div className="mb-5">
                ...
                {isConnected && address ? (
                  <div className="flex items-center gap-2 bg-green-900/20 border border-green-700/30 rounded-xl px-4 py-3">
                    ...
                    <span className="text-green-400 text-xs font-mono truncate">{address.slice(0, 10)}...{address.slice(-6)}</span>
                    <button onClick={() => disconnect()} className="ml-auto text-slate-500 hover:text-red-400 text-xs transition-colors flex-shrink-0">Disconnect</button>
                  </div>
                ) : (
                  <button
              ...
              <div className="border-t border-slate-800 px-5 sm:px-8 py-4 flex items-center justify-between bg-slate-900/50">
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 text-[10px] flex-shrink-0">SDK</div>
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-all flex-shrink-0"
                >
                  Next →
                </button>
              </div>
          ...
          {step === 2 && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="p-5 sm:p-8">
                ...
                <h2 className="text-lg sm:text-xl font-bold text-white mb-2">Configure Your Agent</h2>
                <p className="text-slate-400 text-xs sm:text-sm mb-6 leading-relaxed">
                ...
                <div className="mb-5">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    {agentTypes.map(a => (
                      ...
                      >
                        <div className="text-xl sm:text-2xl mb-1 sm:mb-2">{a.icon}</div>
                        <div className={`text-xs sm:text-sm font-bold mb-0.5 sm:mb-1 ${agentType === a.type ? 'text-indigo-400' : 'text-white'}`}>{a.type}</div>
                        <div className="text-[10px] sm:text-xs text-slate-500 leading-relaxed hidden sm:block">{a.desc}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="border-t border-slate-800 px-5 sm:px-8 py-4 flex items-center justify-between bg-slate-900/50">
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-all"
                >
                  Next →
                </button>
              </div>
          ...
          {step === 3 && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="p-5 sm:p-8">
                ...
                <h2 className="text-lg sm:text-xl font-bold text-white mb-2">Choose Your Plan</h2>
                <p className="text-slate-400 text-xs sm:text-sm mb-6 leading-relaxed">
                  ...
                  {plans.map(p => (
                    ...
                    >
                      ...
                      <div className="flex sm:block items-center justify-between mb-2 sm:mb-0">
                        ...
                        <div className={`text-sm sm:text-lg font-bold sm:mb-3 ${p.popular ? 'text-indigo-400' : 'text-white'}`}>{p.price}</div>
                      </div>
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 flex items-center gap-3 mb-3">
                  <div className="h-8 w-8 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 text-xs font-bold flex-shrink-0">0G</div>
                  <div className="min-w-0">
                    <p className="text-xs text-white font-semibold">Secure Payment via 0G Chain</p>
                    <p className="text-[10px] text-slate-500">On-chain settlement · Transparent · Verifiable</p>
                  </div>
                  {payTxHash && (
                    <span className="ml-auto text-indigo-400 text-xs flex-shrink-0">
                      {paying ? '⏳' : '✅'}
                    </span>
                  )}
                </div>
              ...
              <div className="border-t border-slate-800 px-5 sm:px-8 py-4 flex items-center justify-between bg-slate-900/50">
                ...
                <button
                  ...
                  disabled={paying}
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold px-5 sm:px-8 py-2.5 rounded-xl transition-all"
                >
                  {paying ? '⏳ Processing...' : 'Sign & Subscribe →'}
                </button>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="p-5 sm:p-8 text-center">
                <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-green-900/30 border border-green-700/30 flex items-center justify-center text-2xl mx-auto mb-4">✅</div>
                <p className="text-xs text-green-400 font-semibold uppercase tracking-widest mb-2">Agent Registered</p>
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">{agentName || 'Your Agent'} is Live</h2>
                <p className="text-slate-400 text-xs sm:text-sm mb-6">Your agent has been registered on 0G Mainnet.</p>

                <div className="grid grid-cols-2 gap-3 mb-5 text-left">
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-2">Credit Score</p>
                    <div className="text-4xl sm:text-5xl font-bold text-indigo-400 mb-1">413</div>
                    <div className="text-xs text-slate-500 mb-3">FAIR — Building history</div>
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-2">Pool Balance</p>
                    <div className="text-2xl sm:text-4xl font-bold text-white mb-1">1.80 <span className="text-base sm:text-xl text-slate-500">OG</span></div>
                    <div className="text-xs text-slate-500 mb-3">Available to borrow</div>
                    <button className="w-full bg-indigo-600/20 border border-indigo-500/20 text-indigo-400 text-xs font-bold py-1.5 rounded-lg">
                      Request Credit
                    </button>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={item} className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-green-400 font-semibold flex-shrink-0 ml-2">Active</span>
                      </div>
                    ))}
                  </div>
                </div>

                <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 text-left text-xs space-y-2 mb-5">
                  <div className="flex justify-between gap-2">
                    <span className="text-slate-500 flex-shrink-0">Agent Name</span>
                    <span className="text-white font-mono truncate">{agentName || 'Unnamed'}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                  ...
                  <div className="flex justify-between gap-2">
                    <span className="text-slate-500 flex-shrink-0">Plan</span>
                    <span className="text-white font-mono">{plans.find(p => p.id === plan)?.name ?? plan}</span>
                  </div>
                  <div className="flex justify-between gap-2">
                    <span className="text-slate-500 flex-shrink-0">Network</span>
                    <span className="text-white font-mono text-[10px]">0G Mainnet — 16661</span>
                  </div>
                  {address && (
                    <div className="flex justify-between gap-2">
                      <span className="text-slate-500 flex-shrink-0">Wallet</span>
                      <span className="text-white font-mono text-[10px]">{address.slice(0, 8)}...{address.slice(-6)}</span>
                    </div>
                  )}
                  {payTxHash && (
                    <div className="flex justify-between items-center gap-2">
                      <span className="text-slate-500 flex-shrink-0">Tx</span>
                      <a
                        href={'https://explorer.0g.ai/mainnet/tx/' + payTxHash}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-indigo-400 hover:underline font-mono text-[10px] truncate"
                      >
                        View on 0G Explorer ↗
                      </a>
                    </div>
        ...
        {step === 1 && (
          <p className="mt-4 text-center text-slate-600 text-xs px-4">
            By continuing you agree to the AetherCredit protocol terms.
          </p>
        )}
      </div>
